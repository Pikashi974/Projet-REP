...
const List = document.querySelector("#listesupTechnicien");
...
function renderlist(tasks) {
    tasks.forEach((t) => {
        ...
        <tr>
            ...
            <td>${t.statut}</td>
            <td>     
                <button class="btn btn-secondary btn-sm" onclick="SupprimerTechnicien('${t.id}')">
                    Supprimer
                </button>
            </td>
        </tr>
...
const SupprimerTechnicien = async(id) => {
    const reponse = confirm("Êtes-vous sur de vouloir supprimer ce technicien ?");
    if(reponse){
        await main.SupprimerTechnicien(id); // lancer une demande 
        document.location.href="supprimer_technicien.html";
    }
    else{
        console.log(error);
    }
